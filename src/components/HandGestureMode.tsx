...
export default function HandGestureMode({ enabled, tool, settings, onPointer, onSettingsChange, onClose, onError }: Props) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const hasAndroidBridge = Boolean((window as unknown as { AndroidBridge?: unknown }).AndroidBridge)
      const base = hasAndroidBridge ? new URL('./mediapipe', document.baseURI).href.replace(/\/$/, '') : `${window.location.origin}/mediapipe`
