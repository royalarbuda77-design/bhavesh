...
import {
  Canvas, Circle, Ellipse, FabricImage, FabricObject, Group, IText, Line, PencilBrush,
...
const BoardCanvas = forwardRef<BoardHandle, Props>(function BoardCanvas({
  ...
  useEffect(() => {
    ...
    const pathCreated = (event: { path: FabricObject }) => {
      ...
      mark(event.path as AnnotationObject, isAreaEraser ? 'area-eraser' : currentTool.current === 'shapes' ? 'freehand' : currentTool.current)
      if (isAreaEraser) event.path.set({ globalCompositeOperation: 'destination-out', selectable: false, evented: false })
      commit()
    }
    ...
    canvas.loadFromJSON(page.annotations).then(() => {
      if (disposed) return
      canvas.getObjects().forEach(object => mark(object as AnnotationObject, (object as AnnotationObject).annotationType || 'object'))
      canvas.renderAll()
  ...
  useEffect(() => {
    ...
    canvas.defaultCursor = tool === 'pan' ? 'grab' : tool === 'eraser' ? 'cell' : selectable ? 'default' : 'crosshair'
