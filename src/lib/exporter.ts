...
export async function downloadBlob(blob: Blob, fileName: string) {
  const androidBridge = (window as unknown as { AndroidBridge?: { saveFile: (base64: string, name: string, mimeType: string) => void } }).AndroidBridge
  if (androidBridge) {
    androidBridge.saveFile(await blobAsBase64(blob), fileName, blob.type || 'application/octet-stream')
    return
  }
