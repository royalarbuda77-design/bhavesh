...
export const config = {
  ...
  /**
   * AUTH_MODE=single — personal / no-login mode: every request is treated as
   * the single local owner of the app (auto-provisioned user, login/signup
   * screens redirect straight to the app). Default remains "multi" (secure,
   * full sign-in) for public deployments.
   */
  authMode: process.env.AUTH_MODE === "single" ? ("single" as const) : ("multi" as const),
  appUrl:
    process.env.APP_URL ||
